...
def graph_4D(my_object, g, compMethod, accuracy, generate_SpatDeriv=False, deriv_dim=1):
    V_f = hcl.placeholder(tuple(g.shape), name="V_f", dtype=hcl.Float())
    V_init = hcl.placeholder(tuple(g.shape), name="V_init", dtype=hcl.Float())
    l0 = hcl.placeholder(tuple(g.shape), name="l0", dtype=hcl.Float())
    t = hcl.placeholder((2,), name="t", dtype=hcl.Float())
    probe = hcl.placeholder(tuple(g.shape), name="probe", dtype=hcl.Float())

    # Positions vector
    x1 = hcl.placeholder((g.shape[0],), name="x1", dtype=hcl.Float())
    x2 = hcl.placeholder((g.shape[1],), name="x2", dtype=hcl.Float())
    x3 = hcl.placeholder((g.shape[2],), name="x3", dtype=hcl.Float())
    x4 = hcl.placeholder((g.shape[3],), name="x4", dtype=hcl.Float())
                ...
                with hcl.for_(0, V_init.shape[1], name="j") as j:
                    with hcl.for_(0, V_init.shape[2], name="k") as k:
                        with hcl.for_(0, V_init.shape[3], name="l") as l:
                            ...
                            dV_dx1[0] = (dV_dx1_L.v + dV_dx1_R.v) / 2
                            dV_dx2[0] = (dV_dx2_L.v + dV_dx2_R.v) / 2
                            dV_dx3[0] = (dV_dx3_L.v + dV_dx3_R.v) / 2
                            dV_dx4[0] = (dV_dx4_L.v + dV_dx4_R.v) / 2
                ...
                with hcl.for_(0, V_init.shape[1], name="j") as j:
                    with hcl.for_(0, V_init.shape[2], name="k") as k:
                        with hcl.for_(0, V_init.shape[3], name="l") as l:
                            ...
                            dx_LL1[0] = hcl_math.abs(dx_LL1[0])
                            dx_LL2[0] = hcl_math.abs(dx_LL2[0])
                            dx_LL3[0] = hcl_math.abs(dx_LL3[0])
                            dx_LL4[0] = hcl_math.abs(dx_LL4[0])

                            dx_LU1[0], dx_LU2[0], dx_LU3[0], dx_LU4[0] = my_object.dynamics(t, (x1[i], x2[j], x3[k], x4[l]),
                                                                                            (uOptL1[0], uOptL2[0],uOptL3[0], uOptL4[0]), \
                                                                                            (dOptU1[0], dOptU2[0], dOptU3[0], dOptU4[0]))
                            dx_LU1[0] = hcl_math.abs(dx_LU1[0])
                            dx_LU2[0] = hcl_math.abs(dx_LU2[0])
                            dx_LU3[0] = hcl_math.abs(dx_LU3[0])
                            dx_LU4[0] = hcl_math.abs(dx_LU4[0])

                            # Calculate alpha
                            alpha1[0] = hcl_math.max(dx_LL1[0], dx_LU1[0])
                            alpha2[0] = hcl_math.max(dx_LL2[0], dx_LU2[0])
                            alpha3[0] = hcl_math.max(dx_LL3[0], dx_LU3[0])
                            alpha4[0] = hcl_math.max(dx_LL4[0], dx_LU4[0])

                            dx_UL1[0], dx_UL2[0], dx_UL3[0], dx_UL4[0] = my_object.dynamics(t, (x1[i], x2[j], x3[k], x4[l]),\
                                                                                            (uOptU1[0], uOptU2[0], uOptU3[0], uOptU4[0]), \
                                                                                            (dOptL1[0], dOptL2[0], dOptL3[0], dOptL4[0]))
                            dx_UL1[0] = hcl_math.abs(dx_UL1[0])
                            dx_UL2[0] = hcl_math.abs(dx_UL2[0])
                            dx_UL3[0] = hcl_math.abs(dx_UL3[0])
                            dx_UL4[0] = hcl_math.abs(dx_UL4[0])
                            
                            # Calculate alpha
                            alpha1[0] = hcl_math.max(alpha1[0], dx_UL1[0])
                            alpha2[0] = hcl_math.max(alpha2[0], dx_UL2[0])
                            alpha3[0] = hcl_math.max(alpha3[0], dx_UL3[0])
                            alpha4[0] = hcl_math.max(alpha4[0], dx_UL4[0])

                            dx_UU1[0], dx_UU2[0], dx_UU3[0], dx_UU4[0] = my_object.dynamics(t, (x1[i], x2[j], x3[k], x4[l]),
                                                                                            (uOptU1[0], uOptU2[0], uOptU3[0], uOptU4[0]),\
                                                                                            (dOptU1[0], dOptU2[0], dOptU3[0], dOptU4[0]))
                            dx_UU1[0] = hcl_math.abs(dx_UU1[0])
                            dx_UU2[0] = hcl_math.abs(dx_UU2[0])
                            dx_UU3[0] = hcl_math.abs(dx_UU3[0])
                            dx_UU4[0] = hcl_math.abs(dx_UU4[0])

                            # Calculate alphas
                            alpha1[0] = hcl_math.max(alpha1[0], dx_UU1[0])
                            alpha2[0] = hcl_math.max(alpha2[0], dx_UU2[0])
                            alpha3[0] = hcl_math.max(alpha3[0], dx_UU3[0])
                            alpha4[0] = hcl_math.max(alpha4[0], dx_UU4[0])
